        ...
        <section className="grid gap-6 lg:grid-cols-2 items-center mb-20">
           <div className="relative animate-slide-up">
              <div className="border-l-4 border-red-600 bg-black/80 backdrop-blur-md p-8 shadow-[0_0_50px_rgba(220,38,38,0.2)]">
                ...
                <h1 className="text-5xl md:text-6xl font-bold mb-4 tracking-tighter text-white">Tushar Saini</h1>
                <h2 className="text-xl text-red-500 mb-6 font-mono flex items-center gap-2"><Terminal size={20} /> Offensive Security Engineer</h2>
                ...
                <div className="flex flex-wrap gap-4">
                  <a href="/projects" className="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-black px-6 py-3 font-bold uppercase tracking-widest transition-all">
                    <Terminal size={18} /> View Operations
                  </a>
                  {/* The href must start with / to look in public folder */}
                  <a href="/resume.pdf" download="Tushar_Saini_Resume.pdf" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 border border-red-600 text-red-500 hover:bg-red-600 hover:text-black px-6 py-3 font-bold uppercase tracking-widest transition-all cursor-pointer z-50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {skillGroups.length > 0 ? (
                skillGroups.map((group) => (
                  <div key={group._id}>
                    <h3 className="text-white font-bold mb-3 border-b border-gray-800 pb-1 flex items-center gap-2 text-sm uppercase"><span className="text-red-600">></span> {group.category}</h3>
                    <div className="space-y-3">
                      {group.skills.map((skill) => (
                        <div key={skill._id}>
                          <div className="flex justify-between text-xs text-gray-400 mb-1 font-mono"><span>{skill.name}</span><span>{skill.level}%</span></div>
                          <div className="h-1 w-full bg-gray-900 overflow-hidden"><div className="h-full bg-red-600 shadow-[0_0_10px_red]" style={{ width: `${skill.level}%` }}></div></div>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="text-gray-500 text-sm font-mono col-span-full text-center py-10">Loading database...</div>
              )}
            </div>
          </div>
        </section>
